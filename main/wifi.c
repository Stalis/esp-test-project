...
void wifi_task_fn(void* pvParam) {
    init_wifi();
    for (;;) {
        // TODO: Reconnecting wifi if disconnected
        ESP_LOGI("wifi", "Wifi task is running");
...
TaskHandle_t init_wifi_task(void) {
    ...
    if (xTaskCreate(wifi_task_fn, "wifi", 1024 * 4, NULL, 1, &wifi_task_handler) != pdPASS) {
        ESP_LOGE(TAG, "Error while creating wifi task");
    }

    return wifi_task_handler;
}
